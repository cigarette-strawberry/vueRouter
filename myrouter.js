...
class VueRouter {
  constructor(options) {
    let { routes } = options // 解构赋值 routes

    // this 是 当前类 VueRouter
    ...
    window.addEventListener('hashchange', () => {
      this._route = location.hash.slice(1) // 每次hash改变的时候都会触发这个函数
    })
  }
}
...
VueRouter.install = function (_vue) {
  ...
  Vue.component('router-link', {
    ...
    render(h) {
      ...
      return h(
        ...
        this.$slots.default // this 是 Vue 实例
      )
    }
  })
  ...
  Vue.component('router-view', {
    render(h) {
      ...
      return h(this.$router.routeMap[this.$router._route]) // this 是 Vue 实例
    }
  })
}
